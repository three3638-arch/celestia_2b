...
## 架构总览
下图展示数据库层的整体架构与数据流向，包括 Prisma Schema、配置、客户端注入与类型系统。
...
## 结论
本设计以 Prisma Schema 明确实体关系与约束，配合类型系统与单例客户端实现高效、可维护的数据访问。通过合理的索引、缓存与迁移策略，兼顾性能与可演进性。建议在生产环境中完善审计、归档与安全策略，确保数据生命周期合规与系统稳定。
