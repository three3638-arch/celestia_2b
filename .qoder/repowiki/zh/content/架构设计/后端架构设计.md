...
**更新** 本版本重点反映了应用变更：采用 Prisma ORM + PostgreSQL 16 作为主要数据存储，增强 Prisma 适配器集成，实现完整的认证和授权系统。

## 项目结构
项目采用 Next.js 应用结构，后端逻辑主要集中在以下位置：
- 数据库访问层：通过 Prisma 客户端封装在单例模式中，使用 PostgreSQL 适配器连接数据库。
- 认证与授权：完整的 JWT 令牌管理系统，包括签发、验证、Cookie 设置与清除。
- API 类型与常量：统一的响应格式、分页参数、订单状态映射、货币与语言等常量。
...
Middleware["中间件系统<br/>(src/middleware.ts)"]
end
subgraph "认证与授权"
AuthLib["认证库<br/>(src/lib/auth.ts)"]
JWTConfig["JWT配置<br/>(src/lib/jwt-config.ts)"]
Password["密码处理<br/>(src/lib/password.ts)"]
Validations["输入验证<br/>(src/lib/validations/auth.ts)"]
end
subgraph "数据与基础设施"
...
## 核心组件
- 数据库访问层（Prisma）：以单例模式初始化 Prisma 客户端，使用 PrismaPg 适配器与 PostgreSQL 连接池，开发环境开启查询日志以便调试。
- 认证与授权：完整的 JWT 令牌管理系统，包括签发、验证、Cookie 设置与清除，支持管理员和客户两种角色。
...
- 密码存储：使用 bcryptjs 进行密码哈希存储与校验，盐值轮数为 12。
- 权限控制：基于用户角色（ADMIN/CUSTOMER）与状态（PENDING/ACTIVE）进行访问控制，管理端与普通用户功能边界清晰。
- 中间件设计：在 Next.js 中间件中实现认证中间件，解析 JWT 并注入用户上下文，对敏感接口进行权限校验。
...
- [src/lib/jwt-config.ts:5-9](file://src/lib/jwt-config.ts#L5-L9)
...
- 中间件系统：在中间件层实现统一认证中间件，集中处理 JWT 解析、权限校验与错误处理。
...
- 认证失败
  - 确认 JWT_SECRET 环境变量是否正确配置，检查过期时间与用户状态。
