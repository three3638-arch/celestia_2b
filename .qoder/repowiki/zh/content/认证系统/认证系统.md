...
**更新** 新增了条件域名配置功能，支持生产环境下的动态域名设置，通过 `COOKIE_DOMAIN` 环境变量实现跨域认证，解决了多域名和子域名场景下的认证问题。
...
subgraph "数据层"
H["Prisma 客户端<br/>db.ts"]
I["用户模型<br/>schema.prisma"]
end
subgraph "客户端"
J["登录页面<br/>page.tsx"]
K["注册页面<br/>page.tsx"]
L["管理后台布局<br/>layout.tsx"]
end
subgraph "部署环境"
M["Docker Compose<br/>生产环境"]
N["Nginx 反向代理<br/>HTTPS & 安全头"]
O["环境变量<br/>.env.production"]
end
A --> B
A --> C
A --> D
...
**更新** 新增了条件域名配置功能，通过 `COOKIE_DOMAIN` 环境变量实现动态域名设置，支持生产环境下的跨域认证需求。
...
## 架构总览
认证系统采用"API 路由 + 中间件 + 工具函数 + 条件域名配置"的分层架构：
- API 层负责业务入口与数据持久化
...
**更新** 所有认证相关端点现在都支持条件域名配置，确保跨域场景下的认证 Cookie 正确设置。
...
## 结论
该认证系统以 JWT 为核心，结合中间件与 API 路由实现了完善的用户注册、登录、登出与权限控制。通过 bcryptjs 实现安全的密码存储，借助 httpOnly Cookie 降低 XSS 风险；中间件对 API 与页面路由进行统一保护，支持多角色与多语言场景。新增的条件域名配置功能解决了生产环境下的跨域认证问题，通过动态设置 Cookie Domain 实现多域名共享认证状态。Nginx 反向代理提供 HTTPS 支持和安全头配置，Docker Compose 简化了生产环境部署。建议在生产环境中严格管理 JWT 密钥与 HTTPS 配置，并持续监控与审计认证相关日志。

**更新** 系统现已支持完整的条件域名配置和跨域认证，为多域名和子域名场景提供了完整的解决方案。
