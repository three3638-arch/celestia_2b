...
- **改进注销流程**：使用直接的浏览器导航替代 Next.js 路由器，确保在各种边缘情况下都能可靠地进行页面跳转
...
**更新** 新增了条件域名配置功能，支持生产环境下的动态域名设置，通过 `COOKIE_DOMAIN` 环境变量实现跨域认证，解决了多域名和子域名场景下的认证问题。同时改进了注销流程，使用直接的浏览器导航确保在各种边缘情况下都能可靠地进行页面跳转。
...
subgraph "数据层"
I["Prisma 客户端<br/>db.ts"]
J["用户模型<br/>schema.prisma"]
end
subgraph "客户端"
K["登录页面<br/>page.tsx"]
L["注册页面<br/>page.tsx"]
M["个人资料页面<br/>page.tsx"]
N["待审核页面<br/>page.tsx"]
O["管理后台布局<br/>layout.tsx"]
end
subgraph "部署环境"
P["Docker Compose<br/>生产环境"]
Q["Nginx 反向代理<br/>HTTPS & 安全头"]
R["环境变量<br/>.env.production"]
end
A --> B
A --> C
A --> D
...
- [src/app/[locale]/storefront/profile/page.tsx:1-91](file://src/app/[locale]/storefront/profile/page.tsx#L1-L91)
- [src/app/[locale]/storefront/(auth)/pending/page.tsx:1-102](file://src/app/[locale]/storefront/(auth)/pending/page.tsx#L1-L102)
...
- [docker/nginx/nginx.conf:1-87](file://docker/nginx/nginx.conf#L1-L87)
...
**更新** 新增了条件域名配置功能，通过 `COOKIE_DOMAIN` 环境变量实现动态域名设置，支持生产环境下的跨域认证需求。同时引入了 Server Action 版本的认证操作，提供更可靠的注销功能。
...
**更新** 所有认证相关端点现在都支持条件域名配置，确保跨域场景下的认证 Cookie 正确设置。注销端点现在提供更可靠的注销体验。
...
- **Profile 页面注销**
  - 使用 fetch 发送注销请求
  - 成功后使用 `window.location.href` 进行直接导航
  - 即使网络错误也保持导航到登录页面
- **Pending 页面注销**
  - 提供专门的注销页面，支持直接浏览器导航
  - 即使 API 调用失败也会强制跳转
...
```mermaid
flowchart TD
Start(["点击注销"]) --> Fetch["fetch('/api/auth/logout')"]
Fetch --> Response{"响应成功?"}
Response --> |是| Nav["window.location.href = '/{locale}/storefront/login'"]
Response --> |否| ForceNav["强制导航到登录页面"]
Nav --> End(["注销完成"])
ForceNav --> End
```

**图表来源**
- [src/app/[locale]/storefront/profile/page.tsx:65-81](file://src/app/[locale]/storefront/profile/page.tsx#L65-L81)
- [src/app/[locale]/storefront/(auth)/pending/page.tsx:25-47](file://src/app/[locale]/storefront/(auth)/pending/page.tsx#L25-L47)
...
- 客户端组件
  ...
  - **Profile 页面**：支持注销功能，使用直接浏览器导航
  - **Pending 页面**：提供专门的注销页面
...
**更新** 新增了条件域名配置、Nginx 优化和注销流程优化相关的性能考量。
...
## 结论
该认证系统以 JWT 为核心，结合中间件与 API 路由实现了完善的用户注册、登录、登出与权限控制。通过 bcryptjs 实现安全的密码存储，借助 httpOnly Cookie 降低 XSS 风险；中间件对 API 与页面路由进行统一保护，支持多角色与多语言场景。新增的条件域名配置功能解决了生产环境下的跨域认证问题，通过动态设置 Cookie Domain 实现多域名共享认证状态。Nginx 反向代理提供 HTTPS 支持和安全头配置，Docker Compose 简化了生产环境部署。**最新的改进**包括增强的注销流程，使用直接的浏览器导航确保在各种边缘情况下都能可靠地进行页面跳转，提升了用户体验和系统稳定性。建议在生产环境中严格管理 JWT 密钥与 HTTPS 配置，并持续监控与审计认证相关日志。

**更新** 系统现已支持完整的条件域名配置、跨域认证和增强的注销流程，为多域名和子域名场景提供了完整的解决方案。
...
**更新** 新增了跨域认证、生产环境安全和增强注销流程相关的最佳实践。
