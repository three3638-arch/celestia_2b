...
- **更新**：在商品列表和详情页面中实现了moissanite宝石类型的优先级排序逻辑，确保moissanite在显示顺序中优先于zircon
- **更新**：SKU选择器中主石尺寸显示的UI改进：移除了冗余的'mm'后缀，提升了产品规格显示的一致性和专业性
...
END["排序逻辑<br/>src/lib/actions/product.ts"]
END2["SKU规格优化<br/>src/components/storefront/sku-selector.tsx"]
END3["SKU描述构建<br/>src/app/[locale]/storefront/products/[id]/page.tsx"]
end
subgraph "数据层"
...
## 结论
本仓库已实现完整的商品展示系统，包括商品列表、详情页面、中间件重定向、国际化支持、数据库连接、类型与校验体系、格式化工具与组件库。系统具备良好的扩展性，支持向后兼容性，能够平滑演进URL结构。**最新的moissanite优先级排序逻辑增强了用户体验，确保moissanite宝石类型在显示顺序中优先于zircon，提升了商品展示的专业性和一致性。** 
