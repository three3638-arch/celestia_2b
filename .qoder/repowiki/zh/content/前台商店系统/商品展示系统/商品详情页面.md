...
subgraph "业务与工具"
E["价格计算<br/>decimal.ts"]
F["常量与币种<br/>constants.ts"]
G["产品筛选校验<br/>product.ts"]
H["类型定义<br/>index.ts"]
end
subgraph "数据模型"
I["Prisma Schema<br/>schema.prisma"]
end
A --> B
A --> D
B --> I
...
- 商品数据模型：SPU（Product）含名称、描述、分类、价格区间、图片集合；SKU（ProductSku）含规格（尺寸、链长、材质颜色）、库存状态、参考价；图片（ProductImage）含主图与缩略图URL及排序。
...
### 规格选择交互设计（尺寸/颜色/材质）
- 数据来源：SKU包含gemType（材质）、metalColor（金属颜色）、size（戒指尺码）、chainLength（项链链长）等维度。
...
## 依赖关系分析
- 组件耦合：详情页依赖轮播组件与数据服务；轮播组件不直接依赖业务数据，保持高内聚低耦合。
- 数据模型：Product-Sku-Image三者通过外键关联，SKU承载规格与库存状态，Image承载主图与缩略图。
- 工具依赖：价格计算依赖decimal.ts；币种与默认参数依赖constants.ts；筛选参数依赖product.ts。
...
- 规格选择无效
  - 核对SKU字段映射（gemType/metalColor/size/chainLength）是否与后端一致。
...
## 结论
商品详情页面在当前仓库中尚未提供具体实现文件，但通过轮播组件、数据模型与工具库已具备完整的技术基础。建议按照本文档的架构与实现要点，结合Prisma模型与decimal.ts工具函数，快速构建高性能、可扩展且具备良好用户体验的商品详情页。
