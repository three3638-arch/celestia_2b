# 用户个人中心

<cite>
**本文档引用的文件**
- [README.md](file://README.md)
- [layout.tsx](file://src/app/layout.tsx)
- [storefront-layout.tsx](file://src/components/storefront/storefront-layout.tsx)
- [index.ts](file://src/types/index.ts)
- [db.ts](file://src/lib/db.ts)
- [constants.ts](file://src/lib/constants.ts)
- [utils.ts](file://src/lib/utils.ts)
- [auth.ts](file://src/lib/auth.ts)
- [jwt-config.ts](file://src/lib/jwt-config.ts)
- [middleware.ts](file://src/middleware.ts)
- [route.ts](file://src/app/api/auth/login/route.ts)
- [route.ts](file://src/app/api/auth/register/route.ts)
- [route.ts](file://src/app/api/auth/logout/route.ts)
- [auth.ts](file://src/lib/actions/auth.ts)
- [page.tsx](file://src/app/[locale]/storefront/profile/page.tsx)
- [page.tsx](file://src/app/[locale]/storefront/(auth)/pending/page.tsx)
- [order.ts](file://src/lib/validations/order.ts)
- [schema.prisma](file://prisma/schema.prisma)
</cite>

## 更新摘要
**变更内容**
- 更新了注销功能章节，反映了从Next.js路由器依赖到直接浏览器导航(window.location.href)的改进
- 新增了注销流程的详细说明和可靠性分析
- 更新了相关代码示例和架构图

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介

Celestia用户个人中心是一个基于Next.js构建的现代珠宝电商应用的核心功能模块。该系统提供了完整的用户个人信息管理、订单历史查看、收货地址管理以及用户认证状态管理功能。系统采用现代化的技术栈，包括TypeScript、Prisma ORM、Tailwind CSS等，确保了良好的开发体验和用户体验。

个人中心功能涵盖了用户从注册登录到订单管理的完整生命周期，支持多语言国际化、多币种定价、复杂的订单状态管理和安全的认证授权机制。

## 项目结构

项目采用基于功能的组织方式，个人中心相关的核心文件分布在以下关键目录中：

```mermaid
graph TB
subgraph "应用层"
A[src/app] --> B[API路由]
A --> C[页面组件]
A --> D[布局组件]
end
subgraph "核心库"
E[src/lib] --> F[认证模块]
E --> G[数据库模块]
E --> H[工具函数]
E --> I[常量定义]
end
subgraph "类型定义"
J[src/types] --> K[接口定义]
J --> L[枚举类型]
end
subgraph "数据层"
M[Prisma Schema] --> N[数据库模型]
O[PostgreSQL] --> N
end
A --> E
E --> J
E --> M
M --> O
```

**图表来源**
- [layout.tsx:1-43](file://src/app/layout.tsx#L1-L43)
- [storefront-layout.tsx:1-99](file://src/components/storefront/storefront-layout.tsx#L1-L99)
- [db.ts:1-18](file://src/lib/db.ts#L1-L18)

**章节来源**
- [README.md:1-37](file://README.md#L1-L37)
- [layout.tsx:1-43](file://src/app/layout.tsx#L1-L43)

## 核心组件

### 认证与会话管理

系统实现了基于JWT的认证机制，支持用户注册、登录、登出和会话管理：

```mermaid
classDiagram
class JwtPayload {
+string userId
+string role
+string status
+number iat
+number exp
}
class SessionUser {
+string id
+string phone
+string name
+string role
+string status
+string markupRatio
+string preferredLang
}
class AuthModule {
+signToken(payload) Promise~string~
+verifyToken(token) Promise~JwtPayload|null~
+setAuthCookie(token) Promise~void~
+clearAuthCookie() Promise~void~
+getCurrentUser() Promise~SessionUser|null~
}
JwtPayload --> SessionUser : "转换"
AuthModule --> JwtPayload : "使用"
AuthModule --> SessionUser : "返回"
```

**图表来源**
- [index.ts:41-59](file://src/types/index.ts#L41-L59)
- [auth.ts:1-116](file://src/lib/auth.ts#L1-L116)

### 数据模型与存储

系统使用Prisma ORM管理用户数据，支持灵活的数据查询和关系映射：

```mermaid
erDiagram
USER {
string id PK
string phone UK
string passwordHash
string name
string company
enum role
enum status
decimal markupRatio
string preferredLang
datetime createdAt
datetime updatedAt
}
ORDER {
string id PK
string orderNo UK
string userId FK
enum status
decimal exchangeRate
decimal markupRatio
decimal totalCny
decimal totalSar
datetime createdAt
datetime updatedAt
}
ORDER_ITEM {
string id PK
string orderId FK
string skuId
string productNameSnapshot
string skuDescSnapshot
int quantity
decimal unitPriceCny
decimal unitPriceSar
enum itemStatus
datetime createdAt
datetime updatedAt
}
USER ||--o{ ORDER : "拥有"
ORDER ||--o{ ORDER_ITEM : "包含"
```

**图表来源**
- [schema.prisma:89-106](file://prisma/schema.prisma#L89-L106)
- [schema.prisma:188-220](file://prisma/schema.prisma#L188-L220)
- [schema.prisma:222-247](file://prisma/schema.prisma#L222-L247)

**章节来源**
- [auth.ts:73-116](file://src/lib/auth.ts#L73-L116)
- [db.ts:1-18](file://src/lib/db.ts#L1-L18)
- [schema.prisma:89-106](file://prisma/schema.prisma#L89-L106)

## 架构概览

系统采用分层架构设计，确保关注点分离和可维护性：

```mermaid
graph TB
subgraph "表现层"
A[Next.js App Router]
B[React 组件]
C[UI 组件库]
end
subgraph "业务逻辑层"
D[服务端动作]
E[验证器]
F[业务规则]
end
subgraph "数据访问层"
G[Prisma ORM]
H[数据库适配器]
I[PostgreSQL]
end
subgraph "基础设施层"
J[认证中间件]
K[JWT 管理]
L[缓存策略]
end
A --> B
B --> D
D --> E
D --> F
F --> G
G --> H
H --> I
A --> J
J --> K
K --> L
```

**图表来源**
- [middleware.ts:1-48](file://src/middleware.ts#L1-L48)
- [auth.ts:1-116](file://src/lib/auth.ts#L1-L116)
- [db.ts:1-18](file://src/lib/db.ts#L1-L18)

## 详细组件分析

### 用户认证状态管理

系统实现了完整的用户认证生命周期管理，包括注册、登录、状态验证和会话维护：

```mermaid
sequenceDiagram
participant U as 用户
participant R as 注册API
participant A as 认证模块
participant D as 数据库
participant C as Cookie存储
U->>R : POST /api/auth/register
R->>D : 创建用户记录
D-->>R : 用户ID
R->>A : 签发JWT令牌
A->>C : 设置认证Cookie
C-->>U : 返回认证状态
R-->>U : 注册成功响应
Note over U,C : 用户状态 : PENDING -> ACTIVE
```

**图表来源**
- [route.ts:40-85](file://src/app/api/auth/register/route.ts#L40-L85)
- [auth.ts:10-18](file://src/lib/auth.ts#L10-L18)
- [jwt-config.ts:1-8](file://src/lib/jwt-config.ts#L1-L8)

#### 权限控制机制

系统通过角色和状态双重维度控制用户权限：

| 角色 | 状态 | 可访问功能 | 权限级别 |
|------|------|------------|----------|
| ADMIN | ACTIVE | 系统管理、用户管理、订单管理 | 最高权限 |
| CUSTOMER | ACTIVE | 个人中心、订单查看、地址管理 | 标准权限 |
| CUSTOMER | PENDING | 仅注册、登录 | 限制权限 |

**章节来源**
- [index.ts:42-48](file://src/types/index.ts#L42-L48)
- [middleware.ts:31-48](file://src/middleware.ts#L31-L48)

### 注销功能实现

**更新** 系统提供了两种注销功能实现，均采用了可靠的浏览器导航方式：

#### Profile页面注销功能

Profile页面中的注销功能位于 `src/app/[locale]/storefront/profile/page.tsx` 的第66-81行：

```mermaid
flowchart TD
A[用户点击退出登录] --> B[handleLogout函数执行]
B --> C[发送POST请求到/api/auth/logout]
C --> D{响应状态}
D --> |200 OK| E[清除认证状态]
D --> |错误| F[静默处理错误]
E --> G[window.location.href跳转]
F --> G
G --> H[重定向到登录页面]
```

#### Pending页面注销功能

Pending页面中的注销功能位于 `src/app/[locale]/storefront/(auth)/pending/page.tsx` 的第25-47行：

```mermaid
flowchart TD
A[用户点击退出登录] --> B[handleSignOut函数执行]
B --> C[发送POST请求到/api/auth/logout]
C --> D{响应状态}
D --> |成功| E[window.location.href跳转]
D --> |异常| F[即使失败也跳转]
E --> G[重定向到登录页面]
F --> G
```

#### 注销流程的技术实现

**更新** 两个注销功能都采用了 `window.location.href` 进行页面跳转，这种实现方式相比Next.js路由器依赖具有更高的可靠性：

1. **直接浏览器导航**：避免了Next.js路由器可能存在的状态冲突
2. **强制页面刷新**：确保所有客户端状态被完全清除
3. **跨组件兼容性**：适用于所有类型的组件（客户端和服务端组件）
4. **错误容错性**：即使API调用失败也能保证页面跳转

**章节来源**
- [page.tsx:66-81](file://src/app/[locale]/storefront/profile/page.tsx#L66-L81)
- [page.tsx:25-47](file://src/app/[locale]/storefront/(auth)/pending/page.tsx#L25-L47)
- [route.ts:5-21](file://src/app/api/auth/logout/route.ts#L5-L21)
- [auth.ts:54-68](file://src/lib/auth.ts#L54-L68)

### 订单历史查看界面

订单管理系统提供了完整的订单生命周期管理功能：

```mermaid
flowchart TD
A[进入订单页面] --> B[加载用户订单]
B --> C{订单状态筛选}
C --> |全部| D[显示所有订单]
C --> |待处理| E[显示待处理订单]
C --> |已完成| F[显示已完成订单]
D --> G[订单详情查看]
E --> G
F --> G
G --> H{订单操作}
H --> |查看详情| I[显示订单详情]
H --> |重新购买| J[跳转到商品页面]
H --> |申请售后| K[跳转到客服页面]
I --> L[返回订单列表]
J --> L
K --> L
L --> A
```

**图表来源**
- [constants.ts:1-46](file://src/lib/constants.ts#L1-L46)
- [utils.ts:15-23](file://src/lib/utils.ts#L15-L23)

#### 订单状态管理

系统支持10种不同的订单状态，每种状态都有明确的业务含义和视觉标识：

| 状态代码 | 中文标签 | 英文标签 | 颜色标识 | 业务含义 |
|----------|----------|----------|----------|----------|
| PENDING_QUOTE | 待报价 | Pending Quote | yellow | 等待报价生成 |
| QUOTED | 已报价 | Quoted | blue | 已生成报价单 |
| NEGOTIATING | 协商中 | Negotiating | orange | 客户与供应商协商 |
| CONFIRMED | 已确认 | Confirmed | green | 订单已确认 |
| PARTIALLY_PAID | 部分付款 | Partially Paid | cyan | 已支付部分款项 |
| FULLY_PAID | 已付清 | Fully Paid | emerald | 全款已支付 |
| SHIPPED | 已发货 | Shipped | purple | 商品已发出 |
| SETTLING | 结算中 | Settling | amber | 正在结算处理 |
| COMPLETED | 已完成 | Completed | green | 订单完成 |
| CANCELLED | 已取消 | Cancelled | red | 订单已取消 |

**章节来源**
- [constants.ts:1-46](file://src/lib/constants.ts#L1-L46)
- [order.ts:1-22](file://src/lib/validations/order.ts#L1-L22)

### 收货地址管理功能

地址管理系统支持用户地址的完整生命周期管理：

```mermaid
stateDiagram-v2
[*] --> 地址列表
地址列表 --> 添加地址 : 点击添加
地址列表 --> 编辑地址 : 选择编辑
地址列表 --> 删除地址 : 选择删除
添加地址 --> 地址表单 : 显示表单
地址表单 --> 保存地址 : 提交表单
保存地址 --> 地址列表 : 返回列表
编辑地址 --> 地址表单 : 加载现有数据
地址表单 --> 更新地址 : 提交修改
更新地址 --> 地址列表 : 返回列表
删除地址 --> 确认删除 : 弹出确认对话框
确认删除 --> 地址列表 : 删除后刷新
地址列表 --> 设为默认 : 选择默认地址
设为默认 --> 地址列表 : 更新默认标记
```

**图表来源**
- [storefront-layout.tsx:13-19](file://src/components/storefront/storefront-layout.tsx#L13-L19)

#### 地址数据结构

地址信息包含完整的地理位置和联系信息：

| 字段名 | 类型 | 描述 | 必填 |
|--------|------|------|------|
| id | string | 地址唯一标识 | 是 |
| userId | string | 所属用户ID | 是 |
| fullName | string | 收货人姓名 | 是 |
| phone | string | 联系电话 | 是 |
| country | string | 国家 | 是 |
| province | string | 省份 | 是 |
| city | string | 城市 | 是 |
| district | string | 区县 | 否 |
| streetAddress | string | 街道地址 | 是 |
| postalCode | string | 邮政编码 | 否 |
| isDefault | boolean | 是否默认地址 | 否 |
| createdAt | datetime | 创建时间 | 否 |
| updatedAt | datetime | 更新时间 | 否 |

### 用户信息管理

个人中心提供了全面的用户信息管理功能：

```mermaid
classDiagram
class UserProfile {
+string id
+string phone
+string name
+string company
+string preferredLang
+decimal markupRatio
+string avatarUrl
+Date createdAt
+Date updatedAt
}
class ProfileActions {
+updatePersonalInfo(data) Promise~boolean~
+uploadAvatar(file) Promise~string~
+updateContactInfo(data) Promise~boolean~
+changePassword(data) Promise~boolean~
+updatePreferences(data) Promise~boolean~
}
class AddressManager {
+addAddress(address) Promise~Address~
+editAddress(id, address) Promise~Address~
+deleteAddress(id) Promise~boolean~
+setDefaultAddress(id) Promise~boolean~
+getAddresses() Promise~Address[]~
}
UserProfile --> ProfileActions : "管理"
UserProfile --> AddressManager : "关联"
```

**图表来源**
- [index.ts:50-59](file://src/types/index.ts#L50-L59)
- [storefront-layout.tsx:13-19](file://src/components/storefront/storefront-layout.tsx#L13-L19)

**章节来源**
- [index.ts:50-59](file://src/types/index.ts#L50-L59)

## 依赖关系分析

系统各组件之间的依赖关系清晰明确，遵循单一职责原则：

```mermaid
graph TD
subgraph "认证层"
A[auth.ts] --> B[jwt-config.ts]
A --> C[db.ts]
end
subgraph "API层"
D[login/route.ts] --> A
E[register/route.ts] --> A
F[logout/route.ts] --> A
end
subgraph "业务层"
G[middleware.ts] --> B
G --> A
H[storefront-layout.tsx] --> I[导航组件]
end
subgraph "数据层"
C --> J[schema.prisma]
J --> K[PostgreSQL]
end
subgraph "类型定义"
L[index.ts] --> M[ApiResponse]
L --> N[PaginationParams]
L --> O[SessionUser]
end
A --> L
D --> L
E --> L
G --> L
```

**图表来源**
- [auth.ts:1-116](file://src/lib/auth.ts#L1-L116)
- [jwt-config.ts:1-8](file://src/lib/jwt-config.ts#L1-L8)
- [db.ts:1-18](file://src/lib/db.ts#L1-L18)
- [middleware.ts:1-48](file://src/middleware.ts#L1-L48)

**章节来源**
- [middleware.ts:31-48](file://src/middleware.ts#L31-L48)
- [auth.ts:73-116](file://src/lib/auth.ts#L73-L116)

## 性能考虑

系统在设计时充分考虑了性能优化：

### 数据库优化
- 使用索引优化常用查询字段
- 实现游标分页减少大数据集查询
- 采用连接池管理数据库连接
- 实施查询缓存策略

### 缓存策略
- API响应缓存
- 用户会话缓存
- 静态资源缓存
- 图片资源CDN加速

### 前端优化
- 组件懒加载
- 代码分割
- 图片懒加载
- 防抖节流优化

## 故障排除指南

### 常见问题及解决方案

#### 认证相关问题
- **问题**: 登录后无法访问受保护页面
  - **原因**: JWT令牌过期或Cookie未正确设置
  - **解决**: 检查JWT_SECRET配置，确认Cookie设置参数

- **问题**: 用户状态异常
  - **原因**: 数据库状态字段不一致
  - **解决**: 同步用户状态到ACTIVE

#### 注销功能问题
- **问题**: 注销后仍能访问受保护页面
  - **原因**: 浏览器缓存或Cookie未正确清除
  - **解决**: 使用 `window.location.href` 强制页面刷新，确保Cookie被清除

- **问题**: 注销API调用失败但页面未跳转
  - **原因**: 异常处理逻辑不完整
  - **解决**: 实现异常分支的页面跳转逻辑

#### 数据库连接问题
- **问题**: Prisma连接失败
  - **原因**: DATABASE_URL配置错误
  - **解决**: 验证数据库连接字符串

#### API调用错误
- **问题**: 订单查询返回空结果
  - **原因**: 用户ID不匹配或权限不足
  - **解决**: 验证用户认证状态和查询参数

**章节来源**
- [auth.ts:54-68](file://src/lib/auth.ts#L54-L68)
- [db.ts:9-15](file://src/lib/db.ts#L9-L15)

## 结论

Celestia用户个人中心是一个功能完善、架构清晰的现代化电商应用模块。系统通过合理的分层设计、完善的认证机制和丰富的业务功能，为用户提供了优质的个人中心体验。

主要优势包括：
- 完整的用户生命周期管理
- 灵活的订单状态管理
- 安全的认证授权机制
- 可扩展的架构设计
- 良好的性能表现
- **可靠的注销功能实现**：采用浏览器导航确保注销过程的稳定性

未来可以考虑的功能增强：
- 多语言支持扩展
- 移动端原生应用
- 更丰富的数据分析功能
- AI驱动的个性化推荐

## 附录

### 开发环境设置

1. **环境变量配置**
   - JWT_SECRET: JWT加密密钥
   - DATABASE_URL: 数据库连接字符串
   - NODE_ENV: 环境模式

2. **开发命令**
   ```bash
   npm run dev    # 开发模式
   npm run build  # 生产构建
   npm start      # 启动服务器
   ```

3. **数据库迁移**
   ```bash
   npx prisma migrate dev    # 开发环境迁移
   npx prisma generate       # 生成Prisma客户端
   ```

### API参考

系统提供RESTful API接口，支持标准的HTTP方法和状态码。所有API响应都遵循统一的JSON格式，包含success、data、error和message字段。

**更新** 注销API的详细说明：
- **端点**: `POST /api/auth/logout`
- **功能**: 清除用户认证Cookie并返回注销状态
- **响应**: `{ success: boolean, message?: string, error?: string }`
- **状态码**: 200 成功，500 内部错误