...
- [src/app/[locale]/storefront/orders/[id]/page.tsx](file://src/app/[locale]/storefront/orders/[id]/page.tsx)
- [src/app/admin/orders/[id]/page.tsx](file://src/app/admin/orders/[id]/page.tsx)
</cite>

## 更新摘要
**所做更改**
- 在订单详情页面中新增CUSTOMER_REMOVED状态过滤机制，确保客户看不到已删除的商品项
- 更新订单状态配置，新增CUSTOMER_REMOVED状态定义
- 增强订单商品列表的显示逻辑，对已删除商品进行视觉区分和过滤
- 完善订单删除功能的业务逻辑，防止删除最后一个商品
...
OrderDetailPage["订单详情页面<br/>src/app/[locale]/storefront/orders/[id]/page.tsx"]
end
subgraph "组件层"
...
OrderActions["订单操作<br/>src/lib/actions/order.ts"]
SettlementActions["结算操作<br/>src/lib/actions/settlement.ts"]
end
...
StorefrontLayout --> OrderDetailPage
...
OrderDetailPage --> OrderActions
OrderDetailPage --> SettlementActions
```
...
- [src/lib/actions/settlement.ts](file://src/lib/actions/settlement.ts)
...
- **订单详情页面**：提供完整的订单管理功能，包含CUSTOMER_REMOVED状态过滤机制
...
- **已删除商品过滤**：在订单详情页面中自动过滤CUSTOMER_REMOVED状态的商品项，确保客户界面整洁
...
### 订单详情页面
订单详情页面提供详细的订单信息，现已增强CUSTOMER_REMOVED状态过滤机制：

#### 已删除商品过滤机制
- **自动过滤**：在渲染商品列表时自动过滤itemStatus为CUSTOMER_REMOVED的商品
- **视觉区分**：已删除商品在UI上显示为半透明并带有删除线效果
- **状态标识**：显示"已移除"状态标签，帮助用户理解商品状态
- **操作限制**：已删除商品不再显示删除按钮，防止重复操作
...
#### 删除功能增强
- **完整性检查**：删除前检查订单中是否只剩下一个商品
- **用户提示**：阻止删除最后一个商品，提供明确的错误提示
- **业务逻辑**：如需删除整个订单，用户需使用取消订单功能

```mermaid
sequenceDiagram
participant User as "用户"
participant OrderDetail as "订单详情页面"
participant OrderService as "订单服务"
participant Database as "数据库"
User->>OrderDetail : 点击删除商品
OrderDetail->>OrderDetail : 检查是否只剩一个商品
OrderDetail->>OrderService : 发送删除请求
OrderService->>Database : 更新商品状态为CUSTOMER_REMOVED
Database-->>OrderService : 返回更新结果
OrderService-->>OrderDetail : 确认删除成功
OrderDetail->>OrderDetail : 过滤已删除商品并重新渲染
OrderDetail-->>User : 显示成功提示
```

**图表来源**
- [src/lib/utils.ts:25-31](file://src/lib/utils.ts#L25-L31)
- [src/lib/constants.ts:1-13](file://src/lib/constants.ts#L1-L13)
- [src/app/[locale]/storefront/orders/[id]/page.tsx:218-249](file://src/app/[locale]/storefront/orders/[id]/page.tsx#L218-L249)
- [src/app/[locale]/storefront/orders/[id]/page.tsx:450-452](file://src/app/[locale]/storefront/orders/[id]/page.tsx#L450-L452)
...
- **订单状态翻译**：CUSTOMER_REMOVED状态的多语言显示
...
### 订单系统问题
- **订单状态异常**：检查ORDER_STATUS_CONFIG和ORDER_ITEM_STATUS_CONFIG配置
- **CUSTOMER_REMOVED状态显示**：验证过滤逻辑和UI渲染
- **删除功能失效**：检查删除前的完整性检查逻辑
- **结算数据不显示**：验证结算接口和数据映射
...
- **订单系统增强**：新增CUSTOMER_REMOVED状态过滤机制，提升客户体验
...
- **订单状态维护**：定期检查订单状态配置的准确性和一致性

### 订单系统维护指南
- **状态配置检查**：定期检查ORDER_STATUS_CONFIG和ORDER_ITEM_STATUS_CONFIG的准确性
- **CUSTOMER_REMOVED状态监控**：监控该状态的使用情况和用户反馈
- **删除功能测试**：定期测试删除功能的完整性和安全性
- **UI一致性检查**：确保已删除商品的UI显示符合设计规范
- **国际化验证**：验证CUSTOMER_REMOVED状态在各语言环境下的正确显示