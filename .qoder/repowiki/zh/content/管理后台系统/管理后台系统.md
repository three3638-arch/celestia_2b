...
**更新** 新增Excel导入导出功能，支持批量商品数据处理和模板下载。
...
- [src/components/admin/admin-layout.tsx:1-202](file://src/components/admin/admin-layout.tsx#L1-L202)
...
| 系统设置 | `/admin/settings` | 系统配置和全局设置 |

**章节来源**
- [src/components/admin/admin-layout.tsx:26-44](file://src/components/admin/admin-layout.tsx#L26-L44)
- [src/components/admin/admin-layout.tsx:46-201](file://src/components/admin/admin-layout.tsx#L46-L201)
...
subgraph "后端依赖"
H[prisma] --> H1[ORM框架]
I[@prisma/client] --> I1[数据库客户端]
J[jose] --> J1[JWT处理]
K[bcryptjs] --> K2[密码加密]
L[decimal.js] --> L1[高精度计算]
M[react-dropzone] --> M1[文件拖拽]
end
subgraph "开发工具"
N[typescript] --> N1[类型安全]
O[eslint] --> O2[代码检查]
P[tailwind-merge] --> P1[样式合并]
end
```
...
**更新** 新增的Excel导入导出功能大大提升了商品数据管理的效率，支持批量操作和模板化数据处理，为管理员提供了更加便捷的工作流程。
