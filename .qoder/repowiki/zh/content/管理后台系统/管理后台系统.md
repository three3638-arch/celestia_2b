...
| 商品管理 | `/admin/products` | 商品列表、编辑、创建和库存管理 |
...
- [src/middleware.ts:1-84](file://src/middleware.ts#L1-L84)
...
**图表来源**
- [src/middleware.ts:33-65](file://src/middleware.ts#L33-L65)
...
**章节来源**
- [src/lib/constants.ts:31-46](file://src/lib/constants.ts#L31-L46)
...
subgraph "后端依赖"
E[prisma] --> E1[ORM框架]
F[@prisma/client] --> F1[数据库客户端]
G[jose] --> G1[JWT处理]
H[bcryptjs] --> H2[密码加密]
I[decimal.js] --> I1[高精度计算]
end
subgraph "开发工具"
J[typescript] --> J1[类型安全]
K[eslint] --> K2[代码检查]
L[tailwind-merge] --> L1[样式合并]
end
```
...
#### 基本操作
1. **商品管理**：添加新商品、编辑现有商品信息、管理库存
2. **订单处理**：查看订单详情、更新订单状态、处理客户咨询
3. **客户管理**：查看客户信息、处理客户申请、分析客户行为
