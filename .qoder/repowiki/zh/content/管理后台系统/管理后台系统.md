...
## 更新摘要
**所做更改**
- 更新了订单管理章节，反映了已删除商品处理优化：在管理员订单详情和报价页面中增强了已删除商品的显示和管理功能
- 更新了订单状态配置，增加了CUSTOMER_REMOVED状态的详细说明
- 新增了已删除商品排序、视觉指示和状态管理的详细说明
- 更新了订单计算逻辑，反映了已删除商品的处理方式
...
**更新** 管理后台界面已简化，移除了settings导航项，专注于核心业务功能。新增Excel导入导出功能，支持批量商品数据处理和模板下载。**新增订单已删除商品处理优化**：在管理员订单详情和报价页面中增强了已删除商品的显示和管理功能，包括智能排序、视觉指示和状态管理，提升了订单管理的透明度和用户体验。
...
subgraph "开发工具"
R[typescript] --> R1[类型安全]
S[eslint] --> S2[代码检查]
T[tailwind-merge] --> T1[样式合并]
U[ExcelJS] --> U1[Excel处理]
end
```
